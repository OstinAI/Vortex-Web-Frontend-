...
function openEmployeeModal(emp = null) {
    ...
    if (!modal) return; // Защита от ошибок
    ...
    if (emp) {
        ...
        // Заполнение полей с проверкой на существование
        const fields = {
            'employee-id': emp.id,
        ...
        // Обновляем текст в кастомных селектах
        ...
        if (statusText) statusText.innerText = (emp.status === 'active' ? 'В ШТАТЕ' : emp.status) || 'В ШТАТЕ';
